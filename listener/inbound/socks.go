...
// NewSocks returns a new Socks listener
...
// Config returns the inbound config this listener is configured with
func (s *Socks) Config() InboundConfig {
	return s.config
}

// Close closes the SOCKS listener
...
func (s *Socks) Listen(tunnel adapter.TransportHandler) error {
	var err error
	if s.stl, err = socks.New(s.RawAddress(), tunnel); err != nil {
		return err
	}
	if s.udp {
		if s.sul, err = socks.NewUDP(s.RawAddress(), tunnel); err != nil {
			return err
		}
	}
