...
func createOrGetLocalConn(rAddr, lAddr net.Addr, tunnel tunnel.Tunnel) (*net.UDPConn, error) {
	remote := rAddr.String()
	local := lAddr.String()
	...
	if localConn == nil {
		...
		if loaded {
			...
			if err != nil {
				log.Errorf("listenLocalConn failed with error: %s, packet loss (rAddr[%T]=%s lAddr[%T]=%s)", err.Error(), rAddr, remote, lAddr, local)
				return nil, err
			}
...
func listenLocalConn(rAddr, lAddr net.Addr, tunnel tunnel.Tunnel) (*net.UDPConn, error) {
	...
	go func() {
		log.Debugf("TProxy listenLocalConn rAddr=%s lAddr=%s", rAddr.String(), lAddr.String())
		for {
			...
			if err != nil {
				...
				if errors.Is(err, net.ErrClosed) {
					log.Debugf("TProxy local conn listener exit.. rAddr=%s lAddr=%s", rAddr.String(), lAddr.String())
					return
				}
			}
