...
func NewUDP(addr string, tunnel tunnel.Tunnel, additions ...inbound.Addition) (*UDPListener, error) {
	if len(additions) == 0 {
...
func handlePacketConn(pc net.PacketConn, tunnel tunnel.Tunnel, buf []byte, lAddr, rAddr netip.AddrPort, additions ...inbound.Addition) {
	...
	tunnel.HandleUDPPacket(inbound.NewPacket(target, pkt, proto.Proto_TProxy, additions...))
}
