...
import (
	...
	"github.com/lumavpn/luma/common/cmd"
...
func CleanupTProxyIPTables() {
	...
	log.Warn("Cleanup tproxy linux iptables")
...
func execCmd(cmdStr string) {
	log.Debugf("[IPTABLES] %s", cmdStr)

	_, err := cmd.ExecCmd(cmdStr)
	if err != nil {
		log.Warnf("[IPTABLES] exec cmd: %v", err)
	}
}
