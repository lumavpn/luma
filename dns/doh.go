...
func newDoHClient(urlString string, r *Resolver, preferH3 bool, params map[string]string,
	proxyAdapter proxy.ProxyAdapter, proxyDialer proxydialer.ProxyDialer, proxyName string) dnsClient {
...
func (doh *dnsOverHTTPS) resetClient(ctx context.Context, resetErr error) (client *http.Client, err error) {
	...
	if oldClient != nil {
		...
		if closeErr != nil {
			log.Warnf("warning: failed to close the old http client: %v", closeErr)
		}
	}

	log.Debugf("re-creating the http client due to %v", resetErr)
...
func (doh *dnsOverHTTPS) createTransport(ctx context.Context) (t http.RoundTripper, err error) {
	...
	dialContext := getDialHandler(doh.r, doh.proxyDialer, doh.proxyAdapter, doh.proxyName)
	...
	if slices.Contains(doh.httpVersions, C.HTTPVersion3) {
		...
		if err == nil {
			log.Debugf("[%s] using HTTP/3 for this upstream: QUIC was faster", doh.url.String())
			return transportH3, nil
		}
	}

	log.Debugf("[%s] using HTTP/2 for this upstream: %v", doh.url.String(), err)
...
func (doh *dnsOverHTTPS) dialQuic(ctx context.Context, addr string, tlsCfg *tls.Config, cfg *quic.Config) (quic.EarlyConnection, error) {
	...
	conn, err := listenPacket(ctx, doh.proxyDialer, doh.proxyAdapter, doh.proxyName, "udp", addr, doh.r)
	if err != nil {
		return nil, err
	}
...
func (doh *dnsOverHTTPS) probeH3(
	...
	case tlsErr := <-chTLS:
		if tlsErr != nil {
			...
			log.Debugf("probing TLS: %v", tlsErr)
			return addr, nil
		}
...
func (doh *dnsOverHTTPS) probeQUIC(ctx context.Context, addr string, tlsConfig *tls.Config, ch chan error) {
	...
	log.Debugf("elapsed on establishing a QUIC connection: %s", elapsed)
}
...
func (doh *dnsOverHTTPS) probeTLS(ctx context.Context, dialContext dialHandler, tlsConfig *tls.Config, ch chan error) {
	...
	elapsed := time.Now().Sub(startTime)
	log.Debugf("elapsed on establishing a TLS connection: %s", elapsed)
}
