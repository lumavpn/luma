...
import (
	...
	R "github.com/lumavpn/luma/component/resolver"
...
func withHosts(hosts R.Hosts, mapping *cache.LruCache[netip.Addr, string]) middleware {
	return func(next handler) handler {
		return func(ctx *adapter.DNSContext, r *D.Msg) (*D.Msg, error) {
			...
			ctx.SetType(adapter.DNSTypeHost)
...
func withFakeIP(fakePool *fakeip.Pool) middleware {
	return func(next handler) handler {
		return func(ctx *adapter.DNSContext, r *D.Msg) (*D.Msg, error) {
			...
			ctx.SetType(adapter.DNSTypeFakeIP)
			setMsgTTL(msg, 1)
			msg.SetRcode(r, D.RcodeSuccess)
			msg.Authoritative = true
			msg.RecursionAvailable = true
...
func withResolver(resolver *Resolver) handler {
	return func(ctx *adapter.DNSContext, r *D.Msg) (*D.Msg, error) {
		ctx.SetType(adapter.DNSTypeRaw)
		...
		if err != nil {
			log.Debugf("[DNS Server] Exchange %s failed: %v", q.String(), err)
			return msg, err
		}
