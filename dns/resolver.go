...
func NewResolver(opts *Options) *Resolver {
	c := cache.New(cache.WithSize[string, *D.Msg](4096), cache.WithStale[string, *D.Msg](true))
	defaultResolver := &Resolver{
		main:        transform(opts.Default, nil),
		cache:       c,
		ipv6Timeout: time.Duration(opts.IPv6Timeout) * time.Millisecond,
	}

	r := &Resolver{
		ipv6:  opts.IPv6,
		cache: cache.New(cache.WithSize[string, *D.Msg](4096), cache.WithStale[string, *D.Msg](true)),
		main:  cacheTransform(defaultResolver, opts.Main),
		hosts: opts.Hosts,
	}
