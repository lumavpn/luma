...
import (
	...
	"github.com/lumavpn/luma/common/arc"
...
func (r *Resolver) ExchangeContext(ctx context.Context, m *D.Msg) (msg *D.Msg, err error) {
	...
	if hit {
		log.Debugf("[DNS] cache hit for %s, expire at %s", q.Name, expireTime.Format("2006-01-02 15:04:05"))
		now := time.Now()
		msg = cacheM.Copy()
...
func NewResolver(config Config, m proxydialer.ProxyDialer) *Resolver {
	var cache dnsCache
	...
	defaultResolver := &Resolver{
		main:        transform(config.Default, m, nil),
		cache:       cache,
	...
	LOOP:
		for _, ns := range nameserver {
			...
			// not in cache
			dc := transform([]NameServer{ns}, m, defaultResolver)
			if len(dc) > 0 {
				dc := dc[0]
			...
			if temp := strings.Split(domain, ":"); len(temp) == 2 {
				...
				case "rule-set":
					if p, ok := config.RuleProviders[key]; ok {
						log.Debugf("Adding rule-set policy: %s ", key)
						...
						log.Warnf("Can't found ruleset policy: %s", key)
					}
				case "geosite":
					inverse := false
					...
					log.Debugf("Adding geosite policy: %s inversed %t", key, inverse)
					matcher, err := NewGeoSite(key)
					if err != nil {
						log.Warnf("adding geosite policy %s error: %s", key, err)
						continue
					}
