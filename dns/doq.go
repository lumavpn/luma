...
func newDoQ(resolver *Resolver, addr string, proxyAdapter C.ProxyAdapter,
	proxyDialer proxydialer.ProxyDialer, proxyName string) (dnsClient, error) {
	doq := &dnsOverQUIC{
		addr:         addr,
...
func (doq *dnsOverQUIC) ExchangeContext(ctx context.Context, m *D.Msg) (msg *D.Msg, err error) {
	...
	for i := 0; hasConnection && doq.shouldRetry(err) && i < 2; i++ {
		log.Debugf("re-creating the QUIC connection and retrying due to %v", err)
...
func (doq *dnsOverQUIC) openConnection(ctx context.Context) (conn quic.Connection, err error) {
	...
	rawConn, err := getDialHandler(doq.r, doq.proxyDialer, doq.proxyAdapter, doq.proxyName)(ctx, "udp", doq.addr)
	if err != nil {
	...
	udp, err := listenPacket(ctx, doq.proxyDialer, doq.proxyAdapter, doq.proxyName, "udp", addr, doq.r)
	if err != nil {
		return nil, err
	}
...
func (doq *dnsOverQUIC) closeConnWithError(err error) {
	...
	if err != nil {
		log.Errorf("failed to close the conn: %v", err)
	}
	doq.conn = nil
}
