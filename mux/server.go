...
func (s *Service) NewConnection(ctx context.Context, conn net.Conn, metadata M.Metadata) error {
	...
	if request.Padding {
		...
		return errors.New("non-padded connection rejected")
	}
...
func (s *Service) newConnection(ctx context.Context, sessionConn net.Conn, stream net.Conn, metadata M.Metadata) error {
	...
	if request.Network == "tcp" {
		...
		if request.Destination.Fqdn == BrutalExchangeDomain {
			...
			if err != nil {
				...
				if !features.DebugEnabled {
					err = WriteBrutalResponse(conn, 0, false, errors.Cause(err, "enable TCP Brutal").Error())
					if err != nil {
						return err
					}
					return nil
		...
		var packetConn network.PacketConn
