...
func (c *paddingConn) write(p []byte) (n int, err error) {
	if c.writePadding < kFirstPaddings {
		...
		buffer := pool.NewSize(4 + len(p) + paddingLen)
...
func (c *paddingConn) ReadBuffer(buffer *pool.Buffer) error {
	p := buffer.FreeBytes()
	if c.readRemaining > 0 {
		if len(p) > c.readRemaining {
			p = p[:c.readRemaining]
...
func (c *paddingConn) WriteBuffer(buffer *pool.Buffer) error {
	if c.writePadding < kFirstPaddings {
		bufferLen := buffer.Len()
		if bufferLen > 65535 {
			return util.Error(c.Write(buffer.Bytes()))
		}
		paddingLen := 256 + rand.Intn(512)
...
func (c *vectorisedPaddingConn) WriteVectorised(buffers []*pool.Buffer) error {
	if c.writePadding < kFirstPaddings {
