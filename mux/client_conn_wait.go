...
func (c *clientPacketConn) WaitReadPacket() (buffer *pool.Buffer, destination M.Socksaddr, err error) {
	if !c.responseRead {
		err = c.readResponse()
		if err != nil {
			return
...
func (c *clientPacketAddrConn) WaitReadPacket() (buffer *pool.Buffer, destination M.Socksaddr, err error) {
	if !c.responseRead {
		err = c.readResponse()
		if err != nil {
			return
