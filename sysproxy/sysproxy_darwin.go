...
var (
	networkServices = []string{"Wi-Fi", "Thunderbolt Bridge", "Thunderbolt Ethernet"}
)
...
func EnableSystemProxy(opts *Opts) error {
	log.Debug("Enabling system proxy")
	if opts.EnableWebProxy {
		if err := WebProxySwitch(true, opts.Host, opts.Port); err != nil {
			return err
		}
	}
	if opts.EnableSecureWebProxy {
		if err := SecureWebProxySwitch(true, opts.Host, opts.Port); err != nil {
			return err
		}
	}
	if opts.EnableSocksProxy {
		if err := SocksProxySwitch(true, opts.Host, opts.Port); err != nil {
			return err
		}
	}
...
func DisableSystemProxy() {
	...
	if err != nil {
		log.Errorf("Disable WebProxy failed: %v", err)
	}
	err = SecureWebProxySwitch(false)
	if err != nil {
		log.Errorf("Disable SecureWebProxy failed: %v", err)
	}
	err = SocksProxySwitch(false)
	if err != nil {
		log.Errorf("Disable SocksProxy failed: %v", err)
	}
}
