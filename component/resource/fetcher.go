...
import (
	...
	types "github.com/lumavpn/luma/common"
...
func (f *Fetcher[V]) Initial() (V, error) {
	...
	if stat, fErr := os.Stat(f.vehicle.Path()); fErr == nil {
		...
		if f.interval != 0 && modTime.Add(f.interval).Before(time.Now()) {
			log.Warnf("[Provider] %s not updated for a long time, force refresh", f.Name())
			forceUpdate = true
		}
	} else {
...
func (f *Fetcher[V]) pullLoop() {
	...
	for {
		...
		case <-timer.C:
			...
			if err != nil {
				log.Errorf("[Provider] %s pull error: %s", f.Name(), err.Error())
				continue
			}

			if same {
				log.Debugf("[Provider] %s's content doesn't change", f.Name())
				continue
			}

			log.Infof("[Provider] %s's content update", f.Name())
			if f.OnUpdate != nil {
				f.OnUpdate(elm)
			}
		case <-f.done:
