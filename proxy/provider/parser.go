...
func ParseProxyProvider(name string, mapping map[string]any) (ProxyProvider, error) {
	...
	expectedStatus, err := util.NewUnsignedRanges[uint16](schema.HealthCheck.ExpectedStatus)
	if err != nil {
		return nil, err
	}
	...
	hc := NewHealthCheck([]proxy.Proxy{}, schema.HealthCheck.URL, uint(schema.HealthCheck.TestTimeout), hcInterval, schema.HealthCheck.Lazy, expectedStatus)
