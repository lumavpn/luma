...
import (
	...
	deadline "github.com/lumavpn/luma/conn"
...
func NewConn(c net.Conn, a proxy.ProxyAdapter) C.Conn {
	if _, ok := c.(syscall.Conn); !ok { // exclusion system conn like *net.TCPConn
		c = deadline.NewDeadlineConn(c) // most conn from outbound can't handle readDeadline correctly
	}
...
func newPacketConn(pc net.PacketConn, a proxy.ProxyAdapter) C.PacketConn {
	...
	if _, ok := pc.(syscall.Conn); !ok { // exclusion system conn like *net.UDPConn
		epc = deadline.NewEnhancePacketConn(epc) // most conn from outbound can't handle readDeadline correctly
	}
