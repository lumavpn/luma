...
import (
	...
	"github.com/lumavpn/luma/profile"
...
func (c *CacheFile) SetSelected(group, selected string) {
	...
	if err != nil {
		log.Warnf("[CacheFile] write cache to %s failed: %s", c.DB.Path(), err.Error())
		return
	}
}
...
func initCache() {
	...
	db, err := bbolt.Open(profile.Path.Cache(), fileMode, &options)
	switch err {
	case bbolt.ErrInvalid, bbolt.ErrChecksum, bbolt.ErrVersionMismatch:
		if err = os.Remove(profile.Path.Cache()); err != nil {
			log.Warnf("[CacheFile] remove invalid cache file error: %s", err.Error())
			break
		}
		log.Info("[CacheFile] remove invalid cache file and create new one")
		db, err = bbolt.Open(profile.Path.Cache(), fileMode, &options)
	}
	if err != nil {
		log.Warnf("[CacheFile] can't open cache file: %s", err.Error())
	}
