...
import (
	...
	"github.com/lumavpn/luma/common/ntp"
)
...
func (srv *Service) Start() {
	srv.mu.Lock()
	defer srv.mu.Unlock()
	log.Infof("NTP service start, sync system time is %t", srv.syncSystemTime)
	err := srv.update()
	if err != nil {
		log.Errorf("Initialize NTP time failed: %s", err)
		return
	}
...
func (srv *Service) update() error {
	...
	if offset > time.Duration(0) {
		log.Infof("System clock is ahead of NTP time by %s", offset)
	} else if offset < time.Duration(0) {
		log.Infof("System clock is behind NTP time by %s", -offset)
	}
	if srv.syncSystemTime {
		timeNow := response.Time
		syncErr := setSystemTime(timeNow)
		if syncErr == nil {
			log.Infof("Sync system time success: %s", timeNow.Local().Format(ntp.TimeLayout))
		} else {
			log.Errorf("Write time to system: %s", syncErr)
...
func (srv *Service) loopUpdate() {
	for {
		...
		err := srv.update()
		if err != nil {
			log.Warnf("Sync time failed: %s", err)
		}
	}
}
