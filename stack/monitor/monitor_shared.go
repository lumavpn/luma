...
func (m *defaultInterfaceMonitor) postCheckUpdate() {
	...
	if err != nil {
		log.Errorf("update interfaces: %v", err)
	}
	err = m.checkUpdate()
	if errors.Is(err, ErrNoRoute) {
		...
		log.Errorf("check interface: %v", err)
	}
}
...
func (m *defaultInterfaceMonitor) updateInterfaces() error {
	...
	for _, iif := range interfaces {
		...
		address.addresses = util.Map(util.FilterIsInstance(netAddresses, func(it net.Addr) (*net.IPNet, bool) {
