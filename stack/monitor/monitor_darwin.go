...
func (m *networkUpdateMonitor) loopUpdate() {
	for {
		...
		err := m.loopUpdate0()
		if err != nil {
			log.Errorf("listen network update: %v", err)
			return
		}
	}
}
...
func getDefaultInterfaceBySocket() (*net.Interface, error) {
	...
	if err != nil {
		return nil, errors.Cause(err, "create file descriptor")
	}
	...
	if err != nil {
		return nil, errors.Cause(err, "net.Interfaces")
	}
	for _, netInterface := range interfaces {
		interfaceAddrs, err := netInterface.Addrs()
		if err != nil {
			return nil, errors.Cause(err, "net.Interfaces.Addrs")
		}
	...
	return nil, errors.New("no interface found for address ", selectedAddr)
}
