...
func (o *Options) BuildAndroidRules(packageManager PackageManager, errorHandler errors.ErrorHandler) {
	var includeUser []uint32
	if len(o.IncludeAndroidUser) > 0 {
		o.IncludeAndroidUser = util.Uniq(o.IncludeAndroidUser)
	...
	if len(o.IncludePackage) > 0 {
		o.IncludePackage = util.Uniq(o.IncludePackage)
		for _, packageName := range o.IncludePackage {
			...
			errorHandler.NewError(context.Background(), errors.New("package to include not found: ", packageName))
		}
	}
	if len(o.ExcludePackage) > 0 {
		o.ExcludePackage = util.Uniq(o.ExcludePackage)
		for _, packageName := range o.ExcludePackage {
			...
			errorHandler.NewError(context.Background(), errors.New("package to exclude not found: ", packageName))
		}
	}
}
...
func (o *Options) BuildAutoRouteRanges(underNetworkExtension bool) ([]netip.Prefix, error) {
	...
	if o.AutoRoute && len(o.Inet4Address) > 0 {
		...
		if len(o.Inet4RouteExcludeAddress) == 0 {
			...
			if err != nil {
				return nil, errors.Cause(err, "build IPv4 route address")
			}
	...
	if len(o.Inet6Address) > 0 {
		...
		if len(o.Inet6RouteExcludeAddress) == 0 {
			...
			resultSet, err := builder.IPSet()
			if err != nil {
				return nil, errors.Cause(err, "build IPv6 route address")
			}
			routeRanges = append(routeRanges, resultSet.Prefixes()...)
		}
	}
