...
func (s *System) start(ctx context.Context) error {
	...
	if s.inet4Address.IsValid() {
		...
		s.tcpPort = M.ParseSocksAddrFromNet(tcpListener.Addr()).Port
	...
	if s.inet6Address.IsValid() {
		...
		s.tcpPort6 = M.ParseSocksAddrFromNet(tcpListener.Addr()).Port
		go s.acceptLoop(ctx, tcpListener)
	}
...
func (s *System) acceptLoop(ctx context.Context, listener net.Listener) {
	for {
		...
		connPort := M.ParseSocksAddrFromNet(conn.RemoteAddr()).Port
...
func (w *systemUDPPacketWriter4) WritePacket(buffer *pool.Buffer, destination M.Socksaddr) error {
	newPacket := pool.NewSize(w.frontHeadroom + len(w.header) + buffer.Len())
...
func (w *systemUDPPacketWriter6) WritePacket(buffer *pool.Buffer, destination M.Socksaddr) error {
	newPacket := pool.NewSize(w.frontHeadroom + len(w.header) + buffer.Len())
	defer newPacket.Release()
	newPacket.Resize(w.frontHeadroom, 0)
	newPacket.Write(w.header)
