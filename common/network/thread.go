...
type ThreadSafeReader interface {
	// Deprecated: Use ReadWaiter interface instead.
	ReadBufferThreadSafe() (buffer *pool.Buffer, err error)
}

// Deprecated: Use ReadWaiter interface instead.
type ThreadSafePacketReader interface {
	// Deprecated: Use ReadWaiter interface instead.
	ReadPacketThreadSafe() (buffer *pool.Buffer, addr M.Socksaddr, err error)
}

func IsUnsafeWriter(writer any) bool {
	_, isUnsafe := util.Cast[ThreadUnsafeWriter](writer)
	return isUnsafe
}
...
func IsSafeReader(reader any) ThreadSafeReader {
	...
	if upstream, hasUpstream := reader.(util.WithUpstream); hasUpstream {
...
func CalculateFrontHeadroom(writer any) int {
	...
	for {
		...
		} else if upstream, hasUpstream := writer.(util.WithUpstream); hasUpstream {
...
func calculateReaderMTU(reader any) int {
	...
	for {
		...
		} else if upstream, hasUpstream := reader.(util.WithUpstream); hasUpstream {
