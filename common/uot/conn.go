...
func (c *Conn) WriteTo(p []byte, addr net.Addr) (n int, err error) {
	...
	util.Must(binary.Write(buffer, binary.BigEndian, uint16(len(p))))
	if c.writer == nil {
...
func (c *Conn) WritePacket(buffer *buf.Buffer, destination M.Socksaddr) error {
	...
	util.Must(binary.Write(header, binary.BigEndian, uint16(buffer.Len())))
	if c.writer == nil {
		util.Must1(header.Write(buffer.Bytes()))
		return util.Error(c.Conn.Write(header.Bytes()))
	}
