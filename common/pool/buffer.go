...
func (b *Buffer) Reserve(n int) {
	if n > b.capacity {
		panic(util.ToString("buffer overflow: capacity ", b.capacity, ", need ", n))
	}
	b.capacity -= n
}
...
func (b *Buffer) OverCap(n int) {
	if b.capacity+n > len(b.data) {
		panic(util.ToString("buffer overflow: capacity ", len(b.data), ", need ", b.capacity+n))
	}
	b.capacity += n
}
...
func (b *Buffer) Leak() {
	if debugEnabled {
		...
		if refs == 0 {
			...
			panic(util.ToString("leaking buffer with ", refs, " references"))
		}
	} else {
		b.Release()
	}
