...
func (w *SyscallVectorisedWriter) WriteVectorised(buffers []*pool.Buffer) error {
	w.access.Lock()
...
func (w *SyscallVectorisedPacketWriter) WriteVectorisedPacket(buffers []*pool.Buffer, destination M.Socksaddr) error {
	w.access.Lock()
	defer w.access.Unlock()
	defer pool.ReleaseMulti(buffers)
	var iovecList []windows.WSABuf
