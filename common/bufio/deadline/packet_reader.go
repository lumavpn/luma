...
func (r *packetReader) ReadPacket(buffer *pool.Buffer) (destination M.Socksaddr, err error) {
	select {
	case result := <-r.result:
...
func (r *packetReader) pipeReturnFromBuffer(result *packetReadResult, buffer *pool.Buffer) (M.Socksaddr, error) {
