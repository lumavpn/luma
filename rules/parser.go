...
import (
	...
	RC "github.com/lumavpn/luma/rules/common"
	"github.com/lumavpn/luma/rules/logic"
	RP "github.com/lumavpn/luma/rules/provider"
)

func ParseRule(tp, payload, target string, params []string, subRules map[string][]R.Rule) (parsed R.Rule, parseErr error) {
	switch tp {
	case "DOMAIN":
	...
	case "SRC-PORT":
		parsed, parseErr = RC.NewPort(payload, target, R.SrcPort)
	case "DST-PORT":
		parsed, parseErr = RC.NewPort(payload, target, R.DstPort)
	case "IN-PORT":
		parsed, parseErr = RC.NewPort(payload, target, R.InPort)
	case "DSCP":
