...
type ruleStrategy interface {
	Match(metadata *M.Metadata) bool
	Count() int
...
func (rp *ruleSetProvider) Match(metadata *M.Metadata) bool {
...
func (rp *ruleSetProvider) AsRule(adaptor string) R.Rule {
	panic("implement me")
}
...
func NewRuleSetProvider(name string, behavior P.RuleBehavior, format P.RuleFormat, interval time.Duration, vehicle C.Vehicle,
	parse func(tp, payload, target string, params []string, subRules map[string][]R.Rule) (parsed R.Rule, parseErr error)) P.RuleProvider {
	rp := &ruleSetProvider{
		behavior: behavior,
		format:   format,
	}
...
func newStrategy(behavior P.RuleBehavior, parse func(tp, payload, target string, params []string, subRules map[string][]R.Rule) (parsed R.Rule, parseErr error)) ruleStrategy {
	switch behavior {
	case P.Domain:
